...
#créer un tableau de 100 cases de 0à9 en x et 0 à 9 en y / l'origine est en haut à gauche 
...
#prend un habitant aléatoire de la liste population et le place selon VILAGGERS
...
class KingAndAssassinsClient(game.GameClient):
    ...
    def _nextmove(self, state):
        ...
        if state['card'] is None:
            global assassins
            assassins = IA.ChooseAssassins(state)
            return json.dumps({'assassins': assassins }, separators=(',', ':'))
        else:
                ...
                for i in range(10):
                    for j in range(10):
                        if state['people'][i][j] == 'king':
                            xking = i
                            yking = j      
                Killer = IAtest.CanKill(xking,yking,assassins,state)
